    ...
    fn verify_server_cert(
        ...
        if svid_id_matches(end_entity.as_ref(), &self.expected) {
            ...
            Err(rustls::Error::General(
                "peer certificate does not present the pinned SPIFFE identity".to_owned(),
            ))
        }
    }
...
mod tests {
    ...
    fn a_publicly_valid_but_unpinned_peer_is_rejected_end_to_end() {
        // The server presents a leaf from a CA the config does not trust, standing
        // in for any default/webpki anchor: it must be rejected, proving the config
        // trusts only the pinned CA and no default roots leak through.
