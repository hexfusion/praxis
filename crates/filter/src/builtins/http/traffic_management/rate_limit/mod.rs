...
impl HttpFilter for RateLimitFilter {
    ...
    async fn on_response(&self, ctx: &mut HttpFilterContext<'_>) -> Result<FilterAction, FilterError> {
        ...
        // string (or a cached HeaderValue) would describe the wrong
        // caller.
        #[expect(clippy::cast_possible_truncation, clippy::cast_sign_loss, reason = "burst fits u64")]
        let limit_val = limit.burst as u64;
        if let Some(ref mut resp) = ctx.response_header {
            // Remaining and reset are numeric and convert straight to
            // HeaderValue with no intermediate String and no re-validation
            // byte scan (upstream's hot-path optimisation). The limit varies
            // by caller, so it is converted per response from the resolved
            // limit rather than served from a precomputed value.
            resp.headers
                .insert(&self.header_limit, http::header::HeaderValue::from(limit_val));
            resp.headers
